...
class UniversalAgentBuilderService(looper_basic.BasicService):
    ...
    def update_instance_derivatives(
        ...
        The hook is called when an initiator of updating is an user.
        The default implementation is to delete the outdated derivatives
        and create new ones.
        """

        # The simplest implementation. Update via recreation.
        # Delete the original derivatives
        ...
        # Create nessary derivatives for the new instance
    ...
    def _actualize_updated_instance(
        ...
        if self._instance_model._has_model_derivatives():
            self.update_instance_derivatives(instance, resource, derivatives)
        ...
        self.post_update_instance_resource(instance, resource)
    ...
    def _actualize_updated_instances(self) -> None:
            ...
            derivative_resources = models.TargetResource.objects.get_all(
                filters={
                    "master": dm_filters.In(
                        str(m.uuid) for m in instance_resources
                    ),
                }
            )
        else:
        ...
        derivative_resource_map = {}
        for derivative_resource in derivative_resources:
            derivative_resource_map.setdefault(
                derivative_resource.master, []
            ).append(derivative_resource)

        # Update every resource in accordance with the new secret
        for instance in updated_instances:
            resource = resource_map[instance.uuid]
            der_resources = derivative_resource_map.get(resource.uuid, tuple())

            try:
                self._actualize_updated_instance(
                    instance, resource, der_resources
                )
            except Exception:
                LOG.exception(
    ...
    def _actualize_outdated_instances_no_derivatives(self) -> None:
        ...
        resource_map = self._get_outdated_instance_resources(kind)
