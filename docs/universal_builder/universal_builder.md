...
| Outdate | Derivative objects are changed by some reason. For instance, the derivative object is ready and its status is set to `ACTIVE` | Derivative objects |
| Track | Tracked resources are changed by some reason. For instance, the tracked resource is ready and its status is set to `ACTIVE` | Tracked resources |
